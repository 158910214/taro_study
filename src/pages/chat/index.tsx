  ...
  const sendMessage = () => {
    ...
    Taro.request({
      ...
      success(res) {
        ...
        setMessages((v) => {
          console.log(v)
          return [...messages, { text: res.data.data, sender: 'bot' }]
        })
      },
      fail(err) {
        console.log(err)
      ...
      <View className='messages p-32px'>
        {messages.map((message, index) => (
          <View
            key={index}
            className={`message ${
              message.sender === 'user' ? 'txt-left' : 'txt-right'
            } text-32px mt-18px`}
          >
            {message.sender === 'user' ? (
              <View className='flex'>
                <Image
                  ...
                  src='https://api.iconify.design/raphael:anonymous.svg?color=%23888888'
                />
              ...
              <View className='flex justify-end'>
                ...
                <Image
                  style='width: 20px;height: 20px;background: #fff;'
                  src='https://api.iconify.design/raphael:android.svg?color=%23888888'
                />
              </View>
            )}
          </View>
